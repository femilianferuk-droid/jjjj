...
class AdminForm(StatesGroup):
    ...
    waiting_for_user_id_for_balance = State()  # Новое состояние для ID пользователя при изменении баланса
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        "🎁 *Добро пожаловать в Ru Gifts!*\n\n"
        "✨ Открывайте кейсы, играйте в мини-игры и получайте ценные подарки!\n\n"
...
# ИСПРАВЛЕННЫЙ ОБРАБОТЧИК ИЗМЕНЕНИЯ БАЛАНСА
...
async def admin_change_balance_start(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminForm.waiting_for_user_id_for_balance)

@router.message(AdminForm.waiting_for_user_id_for_balance)
...
@router.callback_query(F.data.in_(["balance_add", "balance_subtract", "balance_set_exact"]))
async def select_balance_type(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_CHAT_ID:
        await callback.answer("⛔ Доступ запрещен!", show_alert=True)
        return
...
@router.message(AdminForm.waiting_for_balance_change)
async def process_balance_change(message: Message, state: FSMContext):
    ...
    try:
        ...
        elif operation == "balance_set_exact":
            old_balance, new_balance = user_data.set_balance(user_id, amount)
...
async def process_broadcast(message: Message, state: FSMContext):
    ...
    progress_msg = await message.answer(f"📤 *Начинаю рассылку...* 0/{total_users}", parse_mode="Markdown")
...
# Остальные обработчики (кейсы, игры, профиль и т.д.)
