    ...
    def update_balance(self, user_id: int, amount: int, update_stats: bool = False):
        ...
        if update_stats:
            if amount > 0:
                ...
                user["total_spent"] += amount  # Для статистики админа
            elif amount < 0:
...
async def process_find_user(message: Message, state: FSMContext):
    ...
    for user_id, user_info in found_users[:10]:  # Ограничиваем 10 результатами
        response += (
            f"🆔 ID: {user_id}\n"
...
@router.message(AdminForm.waiting_for_user_id, F.text)
        ...
        await message.answer(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    ...
                    [InlineKeyboardButton(text="➖ Списать с баланса", callback_data="balance_subtract")],
...
@router.callback_query(F.data.in_(["balance_add", "balance_subtract"]), AdminForm.waiting_for_balance_type)
...
async def main():
    ...
    # Сохраняем ссылку на бота для использования в других функциях
